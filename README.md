...
游戏采用向控制台显示二维地图的形式。玩家显示为字符 `#`，并展示。玩家可以使用 `WASD` 键进行上下左右移动（玩过游戏的都知道我指的是什么，或者可以自行搜索），并可以在前进方向上创建以下物品（若已由物品，则会覆盖原有物品；编辑在只读模式`MapReader`下失效）：
...
下面是默认的命令行（或与其等效，以下认为这些操作的文件为 `main.exe` 或 `MapReader.exe`）：
```
main.exe
```
有三个可选的参数项 `save`、`music` 和 `title`。传递参数项的方式为 `-<key>=<value>`，其中 `<key>` 是参数项名称，`<value>` 是参数项数值。注意参数项数值不能包含空格。

- `save`：将存档的访问与保存位置设为该参数项数值。

- `music`：参数项数值为 `off` 时不播放音乐。

- `title`：设定标题。你的标题将被加上 `[My-Map]` 或 `[MapReader]` 前缀。
